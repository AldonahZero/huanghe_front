...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      const rolls = Math.max(1, Math.floor(shuffleTimes || 1));
      const rand = (set: string) =>
        set.charAt(Math.floor(Math.random() * set.length)) || "";

      chars.forEach((ch: HTMLElement) => {
        const parent = ch.parentElement as HTMLElement | null;
        if (!parent) return;
  ...
  const commonStyle = { textAlign, ...style } as React.CSSProperties;
  const classes = `shuffle-parent ${
    ready ? "is-ready" : ""
  } ${className}`.trim();
  const Tag = (tag as React.ElementType) || "p";
